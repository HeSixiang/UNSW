...
TEST_CASE("awake -> sleep") {
	...
	auto const from = std::string{"awake"};
	auto const to = std::string{"sleep"};
	...
	SECTION("check number of possible path") {
		INFO("the number of path should be 2");
		CHECK(ranges::size(ladders) == 2);
	}

	SECTION("check the size of each path") {
		INFO("the shortest path should be 10");
		for (auto const& path : ladders) {
			CHECK(ranges::size(path) == 10);
		}
	}
