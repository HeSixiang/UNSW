...
TEST_CASE("brewing -> whiskey") {
	auto const english_lexicon = word_ladder::read_lexicon("./english.txt");
	auto const from = std::string{"brewing"};
	auto const to = std::string{"whiskey"};
	...
	SECTION("check number of possible path") {
		INFO("the number of path should be 18");
		CHECK(ranges::size(ladders) == 18);
	}

	SECTION("check the size of each path") {
		INFO("the shortest path should be 30");
		for (auto const& path : ladders) {
			CHECK(ranges::size(path) == 30);
		}
	}
	...
	SECTION("check correctness by using one possible solution") {
		CHECK(ranges::any_of(
		   ladders,
		   testing::contain({"brewing", "crewing", "chewing", "chowing", "choking", "cooking",
		                     "corking", "carking", "parking", "perking", "jerking", "jerkins",
		                     "jerkies", "jerkier", "perkier", "peakier", "beakier", "brakier",
		                     "brasier", "brasher", "crasher", "clasher", "clasper", "clapper",
		                     "clipper", "chipper", "whipper", "whisper", "whisker", "whiskey"})));
	}
